...
namespace OgameWindow {
	...
	void TextBoxMultipleLines::DrawText() {
		...
		text.region(m_TextPenPos).draw(Palette::White);
		text.draw(m_TextPenPos, Palette::Black);
	}
	...
	void TextBoxMultipleLines::Draw() {
		...
		m_ScrollBar.Draw();
		GetClickableRange().drawFrame(1, Palette::Black);
		GetClickableRange().draw(Palette::White);
		if (m_IsSelected) {
			GetClickableRange().drawFrame(1, Palette::Skyblue);
		}
		//m_TextBox.draw(Palette::Red);
		DrawText();
		m_Font(m_Title).draw(GetPosition() + Point(10, -30), Palette::Black);
	}
